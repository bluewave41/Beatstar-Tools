...
class Note {
	constructor(offset, lane, length, resolution) {
		this.offset = offset;
		this.endOffset = offset + length;
		this.resolution = resolution;
		this.beatstarOffset = offset / resolution;
		this.lane = lane;
		this.length = length;
		this.holdTicks = length == 0 ? 0 : Math.floor(Math.floor(this.endOffset / resolution) - Math.floor(this.offset / resolution));
	}
	get beatstarIndex() {
		return this.offset / this.resolution;
	}
	setSwipe(direction) {
		...
		if(this.swipe.holdSwipe && this.length % this.resolution == 0) {
			this.holdTicks--;
		}
	}
	verify() {
